...
class PaymentCheckoutScreen extends StatefulWidget {
  ...
  final String reference; // 1. NUEVA VARIABLE
  ...
  const PaymentCheckoutScreen({
    ...
    required this.reference, // 1.1 REQUERIDA
...
class _PaymentCheckoutScreenState extends State<PaymentCheckoutScreen> {
  ...
  Future<void> _solicitarCotizacion() async {
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Error: Sesión no iniciada")));
      setState(() => _isLoading = false);
      return;
    }

    try {
      // 2. AGREGAR REFERENCE AL INSERT
      final bookingResponse = await supabase.from('bookings').insert({
        ...
        'address_reference': widget.reference, // <--- AQUÍ SE GUARDA LA REFERENCIA LARGA
      }).select().single();

      final bookingId = bookingResponse['id'];

      // ... (El resto del código de muebles y fotos sigue igual, no lo cambié) ...
      ...
      for (var mueble in widget.furnitureItems) {
        ...
        if (mueble['photos'] != null && (mueble['photos'] as List).isNotEmpty) {
          ...
          for (String rutaLocal in rutasFotos) {
            ...
            await supabase.storage.from('evidence').upload(fileName, file, fileOptions: const FileOptions(cacheControl: '3600', upsert: false));
      ...
      if (itemsParaInsertar.isNotEmpty) await supabase.from('booking_items').insert(itemsParaInsertar);
      if (evidenciasParaInsertar.isNotEmpty) await supabase.from('booking_evidence').insert(evidenciasParaInsertar);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            const Text("Resumen de la Solicitud", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            const Text("Revisa los detalles antes de enviarlos.", style: TextStyle(color: Colors.grey), textAlign: TextAlign.center),
                ...
                child: Padding(
                  ...
                  child: ListView(
                    children: [
                      ...
                      const SizedBox(height: 10),
                      _rowInfo(Icons.calendar_month, "Fecha", "$dateStr\n${widget.serviceTime}"),
                      const SizedBox(height: 10),
                      _rowInfo(Icons.location_on, "Dirección", widget.address),
                      const SizedBox(height: 5),
                      // 3. MOSTRAMOS LA REFERENCIA EN EL RESUMEN
                      if (widget.reference.isNotEmpty)
                        Padding(
                          padding: const EdgeInsets.only(left: 32, bottom: 10),
                          child: Text("Ref: ${widget.reference}", style: TextStyle(color: Colors.grey[700], fontSize: 13, fontStyle: FontStyle.italic)),
                        ),
                      ...
                      const SizedBox(height: 10),
                      ...widget.furnitureItems.map((i) => Padding(
                        padding: const EdgeInsets.only(bottom: 8.0),
                        child: Text("• ${i['quantity']}x ${i['type']} (${i['size']})", style: const TextStyle(fontSize: 15)),
                      )),
                    ],
                  ),
                ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0A7AFF), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                child: _isLoading 
  ...
  Widget _rowInfo(IconData icon, String label, String value) => Row(
